...
const ALLOWED = ['https://eval-driven.dev', 'https://www.eval-driven.dev'];
...
describe('grey-cloud snippet', () => {
  it('loads the beacon first-party and does not override send.to', async () => {
    ...
    const snippet = greyCloudBeaconSnippet('insights.eval-driven.dev', 'tok-1');
    assert.match(snippet, /insights\.eval-driven\.dev\/beacon\.min\.js/);
...
describe('rumProxyFetch', () => {
  it('proxies the beacon JS for an allowed origin', async () => {
    ...
    const response = await rumProxyFetch(
      new Request('https://insights.eval-driven.dev/beacon.min.js', {
        headers: { Origin: 'https://eval-driven.dev' },
      }),
      ALLOWED,
      fetchImpl,
    );
    assert.equal(response.status, 200);
    assert.equal(response.headers.get('Access-Control-Allow-Origin'), 'https://eval-driven.dev');
  ...
  it('forwards RUM posts and rejects other origins', async () => {
    ...
    const fetchImpl = async (url, init) => {
      ...
      assert.equal(init.headers.get('origin'), 'https://eval-driven.dev');
    ...
    const ok = await rumProxyFetch(
      new Request('https://insights.eval-driven.dev/rum?v=1', {
        method: 'POST',
        headers: {
          Origin: 'https://eval-driven.dev',
    ...
    const denied = await rumProxyFetch(
      new Request('https://insights.eval-driven.dev/rum', {
        method: 'POST',
