...
| GitHub Pages first-party RUM proxy (`insights.<zone>`) | `edge-dns` when `githubPages` is set (Worker + custom domain; snippet in product HTML) |
...
- Zone `eval-driven-development.dev` + GitHub Pages origin DNS (`githubPages: mzworthington.github.io`) + Web Analytics / RUM site + `insights.` proxy → this repo ([github-pages-origin.md](github-pages-origin.md)).
