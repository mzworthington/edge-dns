...
1. Configure Pulumi/Wrangler with `accountId`, `zoneId`, and hostnames (apex and/or subdomain).
2. Create the Pages (or Workers) project in the **product** stack.
3. Create `DnsRecord` CNAMEs (or equivalent) for those hostnames pointing at the Pages project subdomain.
4. Attach `PagesDomain` (or custom domain bindings) in the product stack.
5. Deploy the app from product CI (`wrangler pages deploy`, etc.).
