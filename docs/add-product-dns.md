...
Agent Lifecycle Kit (`eval-driven-development.dev`): zone + GitHub Pages origin DNS + Web Analytics in `edge-dns` (`githubPages` in [`zones.yaml`](../zones.yaml)). The kit repo runs GitHub Actions Pages deploy and embeds the RUM snippet — see [github-pages-origin.md](github-pages-origin.md).
